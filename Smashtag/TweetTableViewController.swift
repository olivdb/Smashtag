...
class TweetTableViewController: UITableViewController, UITextFieldDelegate
{
    ...
    var searchText: String? {
        didSet {
            ...
            if let term = searchText, !term.isEmpty { RecentSearches.add(term) }
            searchForTweets()
            title = searchText
        }
    }
    ...
    private func setPopToRootButton() {
        if let controllers = navigationController?.viewControllers, controllers.count >= 2 {
            ...
            if let buttons = navigationItem.rightBarButtonItems {
                if !(buttons.flatMap{ $0.action }.contains(#selector(toRootViewController))) {
                    navigationItem.setRightBarButtonItems([toRootButton] + buttons, animated: true)
                }
            } else {
                navigationItem.setRightBarButtonItems([toRootButton], animated: true)
            }
        }
    }
...
//        cell.detailTextLabel?.text = tweet.user.name
    ...
    private struct StoryboardIdentifiers {
        static let mentionSegue = "Show Mentions"
        static let tweetImagesSegue = "Show Images"
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == StoryboardIdentifiers.mentionSegue,
            let tvCell = sender as? TweetTableViewCell,
            let mentionVC = segue.destination as? MentionTableViewController {
            mentionVC.tweet = tvCell.tweet
        } else if segue.identifier == StoryboardIdentifiers.tweetImagesSegue,
            let imagesVC = segue.destination as? ImageCollectionViewController {
            imagesVC.tweets = tweets.flatMap { $0 }
        }
    }
}
